...
export async function handleToolCall(
  ...
  try {
    ...
    // Eliminar duplicados para evitar llamadas extra
    const uniqueSymbols = [...new Set(symbols)];

    // Paralelizar búsqueda de referencias (con límite implícito por el motor MCP)
    ...
    const referencePromises = uniqueSymbols.map(async (symbol) => {
      try {
        ...
        console.warn(`[Omni-Pi] No se pudieron obtener referencias para ${symbol}`);
      }
    });
